...
def obtieneElementoRANDOM(lSolucion,matrix,pesos,rHeuristic,dictcHeuristics,dict,cHeuristic,dictCols):
    ...
    uColumns = []
    if len(uRows) > 0:
        pRows = he.getProposedRows(uRows,rHeuristic,lparam = 10 ) # LPARAM SE QUEDA EN 1 PARA EMPEORAR LA HEURISTICA
        #print ('pRows',pRows)
        for i in range(0,len(pRows)):
            uColumns =  list(set(uColumns + dict[pRows[i]]))
        if len(uColumns) != 0:
            #print(f'len(uColumns) = {len(uColumns)}')
            #print(f'uColumns = {uColumns}')
            
            column = he.heuristByColsRANDOM(pesos,uRows,uColumns,dictCols)
            lSolucion.append(int(column))
        #column = he.heuristByCols(pesos,uRows,uColumns,dictCols)
